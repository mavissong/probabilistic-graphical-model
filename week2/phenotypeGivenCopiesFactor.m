...
copyA = AssignmentToIndex(assignments(:, 2), numAlleles);
copyB = AssignmentToIndex(assignments(:, 3), numAlleles);
indexDominant = AssignmentToIndex(assignments(:, 1), 2);

for i= 1:length(phenotypeFactor.val)
    dominant = indexDominant(i)
    copyAIndex = copyA(i)
    copyBIndex = copyB(i)
    gene = allelesToGenotypes(copyAIndex, copyBIndex)
    if dominant == 1
       phenotypeFactor.val(i) = alphaList(gene)
    else
       phenotypeFactor.val(i) = 1- alphaList(gene) 
    end
end
