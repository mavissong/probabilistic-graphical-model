...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% observe for all factors
% calculate join probability
% marginalization of the result
F = ObserveEvidence(F, E);
for j = 1:length(F)
    F(j).val = F(j).val/sum(F(j).val);
    M = ComputeJointDistribution(F);
end
marginiliazation_target = setdiff(M.var, V);
M = FactorMarginalization(M, marginiliazation_target);
